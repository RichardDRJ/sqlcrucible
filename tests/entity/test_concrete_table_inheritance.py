...
class Dog(Animal):
    ...
    # Redefine all columns for concrete table inheritance
    id: Annotated[UUID, mapped_column(primary_key=True)] = Field(default_factory=uuid4)
...
class Cat(Animal):
    __sqlalchemy_params__ = {
        "__tablename__": "cat",
        "__mapper_args__": {"polymorphic_identity": "cat", "concrete": True},
    }

    # Redefine all columns for concrete table inheritance
    id: Annotated[UUID, mapped_column(primary_key=True)] = Field(default_factory=uuid4)
    type: Annotated[str, mapped_column(String(50))] = Field(default="cat")
    name: Annotated[str, mapped_column()]
