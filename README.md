...
**Unparameterized dicts have limited nested conversion:** An unparameterized `dict` has value type `Any`. When converting to a `TypedDict` with typed fields, the converter cannot prove that `Any` values are compatible with specific field types like nested `TypedDict`s:
...
**Recommendation:** Use parameterized dict types (`dict[str, Any]`, `dict[str, int]`) when you need predictable conversion behaviour with nested structures.

**Required fields are checked at conversion time:** If a target `TypedDict` has required fields that aren't present in the source dict, conversion raises `TypeError`.
